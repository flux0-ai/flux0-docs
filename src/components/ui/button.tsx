...
export const PrimaryButton = ({ children, href, className = '', ...props }) => {
  ...
  return (
    <Component
      {...props}
      {...(href ? { href } : { type: 'button' })}
      className={`relative cursor-pointer no-underline! h-12 overflow-hidden text-sm rounded flex items-center justify-center font-[Sora] bg-gradient-to-tl from-pink-500 via-pink-600 to-pink-700 dark:from-pink-400 dark:via-pink-500 dark:to-pink-600 ring-1 ring-pink-300 dark:ring-pink-400 ring-offset-3 px-5 py-2.5 text-white! dark:ring-offset-neutral-900 transition-all! duration-300! hover:bg-pink-800 hover:dark:bg-pink-700 hover:ring-3 hover:ring-pink-800 hover:dark:ring-pink-700 hover:ring-offset-6 w-[10rem] ${className}`}
    >
      <span className="relative">{children}</span>
    </Component>
  );
